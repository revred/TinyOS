...
- **Another agent may be working this tree at the same time?** Read [`agent/CONCURRENT_SESSIONS.md`](agent/CONCURRENT_SESSIONS.md) — binding, and short. It exists because on 2026-07-28 an unstaged-carefully `git add -A` pushed a broken assurance spine to `main` and put an entire unread Feature into a commit under the wrong authorship. Install the gate it names: `git config core.hooksPath .githooks`.
