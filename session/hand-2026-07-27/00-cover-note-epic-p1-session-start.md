...
3. **`LE-09` proposal** — **filed and decided**: [`Handover 03`](03-le-09-arm64-pi5-slice-proposal.md) scoped the slice and presented two options; the user chose **Option B with the carve-out**. The ARM64 cycle source/timebase starts next (host-testable, needs no board, and it is the piece that tests the harness's arch-neutrality claim — with a Story contract row and Test document first); UART-borne pass/fail folds into `STORY-P1-01-02`; board-dependent pieces wait for `FEAT-P1-02`.
