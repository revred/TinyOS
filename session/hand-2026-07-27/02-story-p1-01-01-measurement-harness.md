...
| LE-09 | Pi 5 (ARM64) is the short-term hardware, but no ARM64 bring-up slice or deploy path exists | Handover 37 directive 1 | Scoping + sequencing decision | Open — **decided 2026-07-27: Option B with the carve-out** ([Handover 03](03-le-09-arm64-pi5-slice-proposal.md)). The ARM64 `CNTVCT_EL0`/`CNTFRQ_EL0` cycle source and timebase start now as host-testable code (it needs no board and is the only thing that tests the harness's arch-neutrality claim); UART-borne pass/fail folds into `STORY-P1-01-02`; boot, PL011 UART and the SD-card run path wait for `FEAT-P1-02`, after which a fault on that board is a reportable event rather than a silent hang. Today's evidence stands behind the urgency either way: Tier 0 p99 noise of 39–61% means no tail claim can be settled under emulation at all |
...
1. **The `LE-09` carve-out, now decided** — the user chose **Option B with the carve-out** on 2026-07-27. First work item: the ARM64 `CNTVCT_EL0` cycle source + `CNTFRQ_EL0` timebase, as a new `FEAT-P1-01` Story (contract row and Test document first, per the spine's no-unmapped-work rule), checked against `hal::time::conformance`, with `cfg(target_arch = "aarch64")` register reads and host-tested arithmetic. Board-dependent pieces wait for `FEAT-P1-02`; see [Handover 03's decision section](03-le-09-arm64-pi5-slice-proposal.md#decision-user-2026-07-27-option-b-with-the-carve-out).
