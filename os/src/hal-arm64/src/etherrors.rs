...
mod tests {
    ...
    fn every_refusal_earns_a_distinct_code_and_health_earns_none() {
        ...
        assert_eq!(blink_code(&refusals[8]), Some(16), "clk-silent counts 16");
        assert_eq!(blink_code(&refusals[9]), Some(17), "clk-enable counts 17");
        assert_eq!(blink_code(&refusals[10]), Some(18), "clk-stuck counts 18");
        assert_eq!(blink_code(&refusals[11]), Some(7));
        assert_eq!(blink_code(&refusals[14]), Some(10));
