...
fn pi5_run(
    ...
    // STORY-P1-09-01 criterion 1: config.txt is generated by the build, so
    // the card's contents stop being folklore. `pi5::CONFIG_TXT` is pinned by
    // test; the card-prep tooling copies this staged file verbatim.
    let config_path = out_dir.join("config.txt");
    std::fs::write(&config_path, pi5::CONFIG_TXT)
