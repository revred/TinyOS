...
| [`STORY-P1-04-05`](../stories/STORY-P1-04-05.md) | The composed scenario under real preemption: a degrade taken while boosted keeps the boost, then lands on its floor, proved by who the dispatcher selected (`LE-50`) | **Verified** (Tier 0, 2026-07-28, `REPORT-2026-07-28-13`; assurance `baseline-debt`) |
...
**What this Feature does not deliver, stated rather than blurred.** Its title names a *deadline monitor* as well as a WCET watchdog. A declared deadline is a different quantity from a declared execution budget, and only the latter is enforced — see `STORY-P1-04-02`'s explicit scope. That gap stays named and unclosed. `LE-22` (degrade and priority inheritance unreconciled) *was* named here as open and not this Feature's to close; it became this Feature's after all, and `STORY-P1-04-04` closed it — with `STORY-P1-04-05` then supplying the Tier 0 behavioural proof (`LE-50`). What remains open in this area is `LE-49`: inheritance is released per task rather than per lock, so a task holding two contended locks drops the second lock's boost when it releases the first.
