...
| [`STORY-P1-09-16`](../stories/STORY-P1-09-16.md) | GEM receive, one frame, fail-closed: a ring of one wrapped descriptor into a **second** pinned region behind a hardware address filter and a MAC-enforced size bound, enable written strictly last, every frame classified by a total function and counted — nothing interpreted | In progress — criteria 1, 2, 3 and 5 host-Green (5, the hand-back, added 2026-08-07 after the wire read `STATE=STOPPED REASON=NOBUFFER`); **criterion 4 closed in full on silicon 2026-08-08** (`20A`, `REPORT-2026-08-08-01`): all five arms, ten frames each, from a settled counter — `ping` and `unicast` moved `accepted` by exactly ten, `ethertype` and `prefix` moved `refused` by exactly ten, and `notforus` moved nothing at all, including `dropped`; criterion 3 amended 2026-08-08 on the owner's ruling — `BNA` is a counted drop, only `OVR` terminal (`LE-118`) |
| [`STORY-P1-09-17`](../stories/STORY-P1-09-17.md) | The admitted verb: a fixed-width `TOS64-CMD/1` envelope classified by a total function over fixed offsets, a two-row deny-by-default answer-only verb table (`PING`, `STATUS`), every refusal spoken on the wire, the answer rate beat-bounded — the moment a received frame first means something, with `-16`'s expiring absence argument re-made rather than cited | In progress — sentence spoken and Story built 2026-08-07; criteria 1, 2, 3 and 5 met (1–3 host-Green, 5 done in writing); **criterion 4 closed on silicon 2026-08-08** — its refusal half in `19A` (three console commands, three named sequence-matched refusals, `M1`) and its **accept half in `20A`** (`M2`: `PING` and `STATUS` answered, `STATUS` replaying the board's own boot verdict, `TOS64-CMD/1 last=STATUS answered=2 lastlen=46`) once `LE-122`'s FCS-in-the-descriptor-length was fixed at the MAC |
...
Remote bytes are data, never code (`RCG-01`). Until 2026-08-07 they were also not
*read* in any sense that mattered — admission compared six payload bytes, a
destination and an EtherType, then incremented a counter — and that absence was
the containment argument, with an expiry stated in its own text.

**The expiry arrived, and the argument is re-made rather than cited.**
`STORY-P1-09-17` lets a received frame mean something, so `C1` gains exactly one
input-derived selection: a verb id chooses a row of a two-row deny-by-default
table, through a walk of the table's own length. Everything else is unchanged —
fixed offsets only, no value from a frame used as an offset, a length or an
address, no parser, no allocation, nothing mapped or launched. The table's rows
are answer-only and disclose only what the beacon already broadcasts, because the
wire peer has no kernel-derived identity (`PD-02`); the answer rate is bounded to
one line per park beat, because an unauthenticated broadcast-capable peer must not
be able to make the board an amplifier (`SEC-20`); and every refusal is spoken on
the wire by name, because a refused command that vanishes reads as a dead board.
`LE-67` records the two grants, and the arming path now *refuses* rather than
assumes when they alias.
...
- ~~**No receive.**~~ **Superseded twice, and dated.** `STORY-P1-09-16`
  (2026-08-06) enabled receive under a four-part containment argument and counted
  frames without interpreting them; `STORY-P1-09-17` (2026-08-07) admitted two
  answer-only verbs. What remains a non-goal is everything above that: no parser,
  no third verb without a charter re-read, no authority-bearing verb, and still no
  board-side discovery of the *host*.
