...
| [`STORY-P1-09-15`](../stories/STORY-P1-09-15.md) | The unclaimed fetch: the root complex's three inbound DMA windows — both register families, twelve dwords derived from the capture — written once inside `establish` and believed only from readback — codes 21–22 | In progress — every criterion Green on silicon 2026-08-04, criterion 5 in its success arm: the beat line walked `STOPPED REASON=TIMEOUT` → `STATE=BEACONING` on the first boot with the twelve dwords, the wire trained at gigabit, the beacon runs every period |
