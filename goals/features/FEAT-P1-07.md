...
Status: **In progress — the ladder is Green on silicon and the Report exists: `BOARD VERDICT 5`–`8` (2026-08-04) put every acceptance criterion of `-03` and `-04` on hardware, `-02`'s decoded-fault clause with them, and the fixture ran to completion; [`REPORT-2026-08-04-01`](../reports/REPORT-2026-08-04-01.md) (filed 2026-08-05) is the first hardware Report and **`LE-09`, `LE-15`, `LE-24` and `LE-27` are closed on it with `closed_in` populated** — the tier exists, and per `ADR 0005` it is a tier, not a bound. Not Complete: no Story is Verified (assurance state stays `baseline-debt` throughout), `-06` criterion 1's machine-parsed wire capture of the envelope is still owed, `-01`/`-05`'s serial criteria have never produced a byte on this bench (`LE-47`), and the ADR 0005 qualification record is unstarted. `-07` is the owner-ordered boot splash (2026-08-03 — display UX, subordinate to the serial evidence); `-08` the owner-ordered ACT-LED execution proof, `-08` and `-09` proven on silicon 2026-08-03**
...
|---|---|---|
| [`STORY-P1-07-01`](../stories/STORY-P1-07-01.md) | AArch64 target spec, boot stub, `EL2 → EL1`, PL011 UART, first byte on the wire | Verified — criteria 1, 2 and 5 met since 2026-07-28; criteria 3 and 4 closed 2026-08-07 on the Ethernet wire per `TEST-P1-07-01-A`'s channel amendment (`LE-47` retired the PL011 path): `current_el=EL2 raw=0x8 now_at=EL1` on two power-cycled boots, the envelope machine-parsed to `verdict ok=true` (`REPORT-2026-08-07-01`) |
| [`STORY-P1-07-02`](../stories/STORY-P1-07-02.md) | Exception vectors; a synchronous fault prints a decoded `ESR_EL1` instead of hanging | In progress — acceptance criterion 2 Green on silicon (`BOARD VERDICT 8`), the vector table, pure decoders and bounded handler met; **the 2026-08-05 pass found the last one unmet in the code**: `Rung::FaultTaken` is declared in both vocabularies and stamped by nothing, so a fault leaves no audit record at all |
| [`STORY-P1-07-03`](../stories/STORY-P1-07-03.md) | Flat identity MMU, Normal cacheable RAM, Device MMIO, caches on — explicitly *not* address spaces | Verified (functional) 2026-08-05 — criteria 2, 3, 4 and 5 Green on silicon; criterion 3's captures are now quoted verbatim in the Test document as it required, and criterion 4's dead-UART clause is substituted onto the GPIO and RP1 device regions with its reason recorded |
| [`STORY-P1-07-04`](../stories/STORY-P1-07-04.md) | GIC + generic-timer periodic tick; `LE-15` resolved by the `PMCCNTR_EL0`/`CNTVCT_EL0` split | Verified (functional) 2026-08-05 — criteria 1, 2, 3, 4 and 5 Green on silicon, discharged by `REPORT-2026-08-04-01`; criterion 3 met in its primary arm and criterion 4 on the sane-value path, both stated |
| [`STORY-P1-07-05`](../stories/STORY-P1-07-05.md) | Host-side run path: SD image build, serial capture, UART pass/fail driving exit codes | In progress — criteria 1, 4 and 5 met; criteria 2 and 3 read the serial line and it has never carried a byte, so no exit code has ever been driven by a board capture |
| [`STORY-P1-07-06`](../stories/STORY-P1-07-06.md) | `fixture_measure` on the board, batched-iteration measurement, the first hardware Report | Verified (functional) 2026-08-05 — criteria 2, 3, 4, 5 and 6 discharged by `REPORT-2026-08-04-01`; **criterion 1 met off the wire 2026-08-05** (11 metrics parsed by the unchanged parser from a live capture, evidence `goals/reports/wire-meas-envelope-2026-08-05.txt`). Assurance state stays `specified`: not release-assured, 0 qualified platforms |
| [`STORY-P1-07-07`](../stories/STORY-P1-07-07.md) | The boot splash: "TinyOS" on HDMI via the firmware mailbox framebuffer, after the verdict, bounded, hostile-input-validated — UX, never evidence | In progress — criteria 1, 2 and 3 met, criterion 2 exercised in its refusal arm on silicon; criteria 4 and 5 need a firmware that grants the exchange and this one refuses it, so the Story may not be closeable on this hardware |
| [`STORY-P1-07-08`](../stories/STORY-P1-07-08.md) | The ACT LED: forced on at entry, 1 Hz in the park loop — execution as a naked-eye fact through the one device behind no suspect peripheral; an instrument, never evidence | Verified (functional) 2026-08-05 — every criterion Green on silicon 2026-08-03, the lamp re-observed on every boot since including the 2026-08-05 netbooted run; polarity measured active-HIGH against the vendor listing |
| [`STORY-P1-07-09`](../stories/STORY-P1-07-09.md) | The firmware's canvas: the captured 1920×1080 RGB565 simple-framebuffer painted through the existing Surface seam — the report as on-screen text; mailbox path untouched beside it | Verified (functional) 2026-08-05 — every criterion Green on silicon 2026-08-03; criterion 4 is the channel `BOARD VERDICT 1`–`9` reached a human through, serial being dead |
| [`STORY-P1-07-10`](../stories/STORY-P1-07-10.md) | Interrupt masking becomes a scope rather than a one-way switch — the fixture's mask had no counterpart, so the tick died at the fixture and `-04` criterion 1 was unreachable on every board | Verified (functional) 2026-08-05 — host half Green 2026-08-04; criteria 4 and 5 Green on silicon (`BOARD VERDICT 7`), independently corroborated by `BOARD VERDICT 8`'s `SPSR.I=0`, `LE-71` closed |
