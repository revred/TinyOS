...
All Stories **Verified**: the harness produces stable percentile evidence for D04/D05/D07 under Tier 0, baselines are committed, the CI gate runs on every PR, and a deliberately-introduced regression has been demonstrated to fail it. `STORY-P1-01-03` adds no hardware requirement to this Feature's exit: it is the arch-seam check, not the hardware tier, which stays `LE-09`'s debt.

**One qualification this criterion has to carry, added by `STORY-P1-01-04` (`LE-24`).** Of the two D07 metrics, `pool_u64x64_alloc_free_round_trip` is **measured but no longer gated**: it medians to 0 cycles, because the operation costs less than the calibrated `rdtsc` overhead subtracted from it, so what it reports is quantisation rather than evidence. `pool_u64x4_alloc_denied_exhausted` remains gated, as do both D05 metrics, D04 and D02. So "stable percentile evidence for D04/D05/D07" is true of five of the six measured metrics and is **not** true of that one, and this Feature's exit is claimed on that basis rather than on the sentence above read literally. Fixing it needs either a batched-iteration measurement shape — time N operations and divide — or `LE-09`'s board; ungating it is containment, not a repair.
