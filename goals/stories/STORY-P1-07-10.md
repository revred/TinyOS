...
Status: **Verified (functional) 2026-08-05 — all five acceptance criteria met. Host half Green 2026-08-04 (7 new tests in `hal::interrupts`, Red first against the defect's own behaviour: the recording gate logged `[Masked, BodyRan]` with no `Restored`, which is `LE-71` exactly). **Criteria 4 and 5 Green on silicon** — `BOARD VERDICT 7` (measure boot 2026-08-04, kernel `619f40b8c076`) read `TOS64-TICK/1 count=1816 rmin=999 rmax=1000`, the tick surviving the fixture and handing [`STORY-P1-07-04`](STORY-P1-07-04.md)'s ratio requirement its evidence. Criterion 5 confirmed by a test written to refute it: Verdict 6's lone `max=3519` outlier is **absent** at `max=376`, on the boot where the tick fires 100×/s for the whole park loop, and the distributions came back tighter rather than looser. `LE-71` closed.

**Advanced to `Verified` (functional) 2026-08-05** in [`06A`](../../session/hand-2026-08-05/06A-nothing-is-verified-and-the-reason-is-not-velocity.md) §4.1's closing pass. Criteria 1, 2 and 3 are host-provable by construction and were driven Red first against the real defect's behaviour rather than against a hypothetical one, which is the distinction that makes them evidence. An independent corroboration arrived from a Story that was not asked: `BOARD VERDICT 8`'s fault frame recorded `SPSR=0x…0345`, and `SPSR.I=0` says interrupts were **unmasked at the moment of the fault** — read off a code path that had no stake in this Story's claim. **This Story is why `STORY-P1-07-04` criterion 1 was reachable at all**: `LE-71` was structural, not intermittent — the fixture's mask had no counterpart, so the tick's whole lifetime was the ~10 ms before the fixture, which at 100 Hz admits exactly one tick, and one tick is zero intervals. A criterion asking for a ratio was therefore unsatisfiable on any board, however many times it was booted, and no host gate could see it. **Assurance state remains `specified` and this Story is NOT release-assured**: 0 qualified platforms.**
