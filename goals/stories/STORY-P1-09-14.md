...
Status: **Verified (functional) 2026-08-05 — every criterion Green on silicon 2026-08-04, criterion 4 answered first boot: the spoken verdict read `STATE=STOPPED REASON=TIMEOUT` — one word cleared the watch and the MDIO path (a `Stopped` is only reachable through a resolved watch) and convicted the transmit, whose DMA inbound path is the next story. This Story is the reason the previous three park states stopped being one undifferentiated `PARKED`, and its value is measurable in what it saved: the next fix was chosen from a single word rather than from three overlapping conjectures, and `STORY-P1-09-15` was written against the RC inbound windows the same night. Criterion 1's remaining distinct states were subsequently spoken on later boots — `STATE=BEACONING` at `BOARD VERDICT 4` and again on 2026-08-05's netboot run — so more than one arm of the exhaustive line has been read off hardware. Advanced under [`06A`](../../session/hand-2026-08-05/06A-nothing-is-verified-and-the-reason-is-not-velocity.md) §4.1. **Assurance state remains `specified` and this Story is NOT release-assured**: 0 qualified platforms.**
