...
Status: **In progress — host-testable half Green 2026-08-04 (283 host tests in `hal-arm64`, from 275, Red first: GIC enable order and refusals against latching doubles, ratio arithmetic, the `LE-15` decision function with its fallback arm exercised, every report line pinned to exact bytes, and the `LE-69` priority-width discovery against narrow-interface doubles). **Acceptance criteria 2, 3, 4 and 5 Green on silicon** — `BOARD VERDICT 5` (measure boot 2026-08-04, kernel `0c709197ed26`) recorded `TOS64-CONF/1 cntvct=pass span=168 cntfrq=54000000 cpus=54` and `TOS64-PMU/1 delta=24000227 rate=2400mhz source=pmccntr`, closing `LE-27` on the register run and resolving `LE-15` by measurement to `PMCCNTR_EL0` at the A76's 2.4 GHz core clock. **Criterion 1 remains Red and is now blocked on `LE-69`, not on bench access**: the same boot read `TOS64-TICK/1 refused=gicc-pmr readback=000000F0`, a constant asserting five implemented priority bits refusing a conforming four-bit GIC-400. The gate has been rewritten to discover the width from the device; criterion 1 needs one re-boot carrying that fix. The evidence channel is the canvas (`TOS64-TICK/1` live, `TOS64-CONF/1` and `TOS64-PMU/1` at boot) — serial has never produced a byte on this bench (`LE-47`). Not Verified.**
