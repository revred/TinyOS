...
Status: **Verified (functional) 2026-08-05 — host-testable half Green 2026-08-04 (283 host tests in `hal-arm64`, from 275, Red first: GIC enable order and refusals against latching doubles, ratio arithmetic, the `LE-15` decision function with its fallback arm exercised, every report line pinned to exact bytes, and the `LE-69` priority-width discovery against narrow-interface doubles). **Acceptance criteria 2, 3, 4 and 5 Green on silicon** — `BOARD VERDICT 5` (measure boot 2026-08-04, kernel `0c709197ed26`) recorded `TOS64-CONF/1 cntvct=pass span=168 cntfrq=54000000 cpus=54` and `TOS64-PMU/1 delta=24000227 rate=2400mhz source=pmccntr`, closing `LE-27` on the register run and resolving `LE-15` by measurement to `PMCCNTR_EL0` at the A76's 2.4 GHz core clock. **Criterion 1 remains Red, and two distinct defects had to be cleared to see why.** `BOARD VERDICT 5` read `TOS64-TICK/1 refused=gicc-pmr readback=000000F0` — a constant asserting five implemented priority bits refusing a conforming four-bit GIC-400 (`LE-69`, fixed by discovering the width from the device). `BOARD VERDICT 6`, the first boot carrying that fix, read `COUNT=1 TVAL=540000 RMIN=NONE RMAX=NONE`: the whole GIC ladder proven — distributor, PPI 27, priority mask, CPU interface, `CNTV_CTL_EL0` enabled-and-unmasked, one interrupt delivered, vectored, counted and retired — but the tick never repeated. `LE-71` is why, and it is structural rather than intermittent: `fixture_measure` masked interrupts with no counterpart, so the tick's entire lifetime was the ~10 ms window before the fixture, which at 100 Hz admits exactly one tick. One tick is zero intervals, and this criterion asks for a ratio, so it was unreachable on any board however many times it was booted. Fixed by [`STORY-P1-07-10`](STORY-P1-07-10.md). **`BOARD VERDICT 7` closes it: `TOS64-TICK/1 count=1816 tval=540000 rmin=999 rmax=1000`** — ~18 s of uptime at the declared 100 Hz, with every pair of consecutive intervals agreeing to within 0.1%, judged by ratio and never by absolute value exactly as this criterion required. **All five acceptance criteria are now Green on silicon.** The evidence channel is the canvas (`TOS64-TICK/1` live, `TOS64-CONF/1` and `TOS64-PMU/1` at boot) — serial has never produced a byte on this bench (`LE-47`).

**Advanced to `Verified` (functional) 2026-08-05** in [`06A`](../../session/hand-2026-08-05/06A-nothing-is-verified-and-the-reason-is-not-velocity.md) §4.1's closing pass. Every criterion is discharged by [`REPORT-2026-08-04-01`](../reports/REPORT-2026-08-04-01.md), including the two that are easiest to overclaim: criterion 3 was met in its **primary** arm (`PMCCNTR_EL0` enabled and readable, `DELTA=24000227 RATE=2400MHZ`) rather than its fallback, so the "if it traps or reads zero" branch is untested on this board and nothing here says otherwise; and criterion 4's honest-absence path was **not** exercised, because this firmware *does* program `CNTFRQ_EL0` — the criterion asks that the behaviour "survive silicon", and what silicon showed is the sane-value path, with the zero/implausible arms still host-only. Both are stated rather than smoothed over. Criterion 5's recorded decision is in the Report's `LE-15` row with `closed_in` populated. **Assurance state remains `specified` and this Story is NOT release-assured** — the Report is explicit that no `PERF-*` gate closes on it and that `ADR 0005` bars any `G04` bound claim while `qualified-platforms.tsv` holds zero qualified platforms (`06A` §2).**
