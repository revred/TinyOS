...
2. **Demonstrated on the real fixture, in all four quadrants.** **Met**, and this is the Story's evidence:
   ...
   **Scope, stated because it is easy to over-read**: these four rows vary **load on one machine**. They do not test a second machine, and ratio invariance under load does not imply ratio invariance across microarchitectures — the reference is ALU-bound and `D05/dispatch_select` is memory- and branch-bound, so a host that scales those differently shifts the ratio with no regression present. That is `LE-23`, and it is the outcome to rule out rather than the surprise case.
