...
Status: **Verified (functional) 2026-08-05 — all four acceptance criteria met. Criteria 1, 2 and 3 host half Green 2026-08-03 (the five-register mapping pinned against the on-silicon capture, window-class refusals program once and revalidate, link-class refusals never write, the second verdict is final). **Criterion 4 Green on silicon** in both of its arms: `BOARD VERDICT 1` counted 16 — a rung well beyond the window's 4, so the window rung was passed rather than merely not reached — and `BOARD VERDICT 2` reached the plain pulse with `RP1=PRESENT ID=0x0109 PHY=0x600D84A2`. Either way, as the criterion says, the window rung is closed on silicon. Criterion 1's arithmetic earned an independent confirmation this Story could not have arranged: the 2026-08-03 ~23:59 Pi OS inbound-path capture read the live PCIE2 controller registers under working Ethernet and found `WIN0_LO=0 WIN0_HI=0 BASE_LIMIT=0xfff00000 BASE_HI=0x1f LIMIT_HI=0x1f` — **identical to this Story's pinned constants**, so the mapping was verified against a working system rather than only against the dmesg line it was derived from. **Assurance state remains `specified` and this Story is NOT release-assured**: 0 qualified platforms.**
