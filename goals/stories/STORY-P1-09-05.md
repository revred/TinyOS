...
Status: **In progress — criteria 1, 2 and 3 met; **criterion 4's missing thing is that neither of the two arms it names has actually been observed — the outcome arrived through a third channel the criterion does not mention.** Criteria 1, 2 and 3 host half Green 2026-08-03 (heartbeat line pinned, fail-safe stop proven, bounce pure and in-bounds, verdict color pinned). Criterion 4 is written as a disjunction: *either* a host serial listener receives repeating heartbeats, *or* a monitor connected from power-on shows the bouncing block moving. **Neither happened.** Serial has never produced a byte on this bench (`LE-47`), and no capture or photograph on record shows the block in motion — `BOARD VERDICT 4` records "slow blue fill, then TinyOS title" and the report rows, with nothing about the block. What *did* reach a human, on every boot from `BOARD VERDICT 1` onward and again on 2026-08-05's netbooted run, is the **heartbeat line as text on the canvas** — `TOS64-BEAT/1 SEQ=133,134,… STATE=BEACONING FB=REFUSED` — incrementing live, read off the screen by the owner. That is this Story's `TOS64-BEAT/1` bytes (criterion 1) arriving through `STORY-P1-07-09`'s console rather than through either channel criterion 4 anticipated, and the criterion's stated purpose — "something reaches a human without being asked for twice", turning four timed silences into an untimed experiment — is satisfied in substance. **It is deliberately not being claimed as met on that basis.** The house rule is that a Test document records a channel substitution rather than absorbing it silently, and no such amendment has been written here; `TEST-P1-09-05-A` still names the serial listener and the moving block. The `fb=refused` half *is* discharged as written — the criterion asks that a dark screen under those conditions be recorded as evidence against the mailbox path, and `FB=REFUSED` on every boot is exactly that, now corroborated by `STORY-P1-07-09`'s canvas painting where the mailbox refused. Closing this needs one of: a photograph of the block moving, or an amendment to `TEST-P1-09-05-A` substituting the canvas heartbeat with its reason stated. Not Verified.**
