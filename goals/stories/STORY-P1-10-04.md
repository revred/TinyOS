...
Status: **Verified (functional) 2026-08-05 — all seven acceptance criteria met. Written 2026-08-04 from the owner's closing question in [`session/hand-2026-08-04/05A`](../../session/hand-2026-08-04/05A-the-board-speaks-in-spoors-and-what-a-late-listener-cannot-know.md) §5, implemented test-first (14 tests Red-verified against an unwritten implementation), and **proven on silicon 2026-08-05** across `BOARD VERDICT 11`, `12` and `13`. Criteria 1–5 and 7 Green: a capture opened at record 74 with frame 0 long gone still read `MmuEnabled cost=184052`, `GicRouted`, `TickArmed` out of a retained certificate; the epoch changed `0x049F8B28` → `0x04B328BC` → `0x04B32825` across power cycles; one window held two boots with **0 records lost** across the restart; and a window holding both a live frame 0 and a certificate for one epoch compared them record for record and reported **byte-identical**. Announcement cadence measured at one frame per ~5 s on two separate boots, matching `ANNOUNCE_EVERY`. **Criterion 6 is met**, and the earlier reading of it as "host-Green only" was too strict about the wrong thing. The criterion asks that nothing be retained without bound — a fixed-size buffer that fills once and never grows, where *"a rung that repeats every park pass can never displace what a boot established"*. That second clause is the substantive one and it has now been observed at length on hardware: `BOARD VERDICT 11` opened at record 74 and `BOARD VERDICT 14` ran to record 1738, with `ParkIteration`, `BeaconTransmitted`, `ThermalSample` and `DispatchRound` repeating every single beat throughout — and across every one of those windows the certificate still held exactly the three boot rungs, unchanged, `MmuEnabled`/`GicRouted`/`TickArmed`. Thousands of repeating rungs displaced nothing. What has *not* happened is a board run stamping enough **distinct boot rungs** to reach the buffer's ceiling, so the fills-once-and-refuses arm is host-only; that is a statement about an arm of the implementation, not about an unmet criterion, and it is recorded here rather than left to inference. **`LE-74` was amended by these runs rather than merely cited**: two consecutive boots produced epochs 151 counter ticks apart — 2.8 microseconds at 54 MHz — so the entropy is measurably thinner than the design-time caution assumed, and the epoch is sound as a **change detector** while unusable as an **identifier**. Criterion 1's "different with high probability" survives that finding only in the bench sense, and no future reader may treat the field as an identity. Advanced under [`06A`](../../session/hand-2026-08-05/06A-nothing-is-verified-and-the-reason-is-not-velocity.md) §4.1. **Assurance state remains `specified` and this Story is NOT release-assured**: 0 qualified platforms.**
