...
Status: **In progress — criteria 1, 4 and 5 met; **criteria 2 and 3's missing thing is that the exit code has never been driven by a real capture, because the channel it reads has never carried a byte.** Host half Green 2026-07-30 (31 host tests, Red first; the one command builds `kernel8.img` and prints placement). Criterion 1 is met and has been exercised on every board image since: one command builds a placeable image and prints where the artifacts go, and nothing about the image is folklore. Criterion 4 is met — the path appears in `list-fixtures` with its owning `TEST-*`, per `STORY-P0-01-04`'s rule. Criterion 5 is met: capture parsing, verdict extraction, timeout handling and exit-code mapping are pure functions over captured text, with only the port open as I/O. **Criteria 2 and 3 read the serial line**, which has produced nothing on this bench (`LE-47`), so the pass/fail protocol has never driven an exit code from a board and the three outcomes criterion 3 separates — never spoke, spoke and stopped, reported failure — have never been distinguished against real hardware. **The board loop this Story exists to serve now exists and does not run through it**, which is the honest state and is worth recording rather than leaving as an apparent gap: `FEAT-P1-11`'s netboot path plus `ti64dink --until` is what actually closed a board run to a machine-checked condition on 2026-08-05, exit 0 on sighting and 1 on timeout — the same three-outcome discipline criterion 3 specifies, over the wire instead of the wire's intended replacement. Closing this Story as written needs a working adapter; closing its *purpose* may instead mean an amendment routing criteria 2 and 3 to the capture path that works. **Decided 2026-08-07 on the owner's instruction (`hand-2026-08-07/09A`): the amendment route.** The serial half is retired with `LE-47` rather than waited on — the same fork `STORY-P1-07-01` resolved the same day on the `TEST-P1-07-03-A` precedent — and criteria 2 and 3's purpose closes on the wire path, which now demonstrates two of its outcomes live: exit 0 on a parsed verdict (`xtask parse-meas`, `verdict ok=true`, 2026-08-07) and exit 1 on timeout (`ti64dink --until`, 2026-08-05). What keeps this Story **In progress** rather than Verified is stated exactly: `TEST-P1-07-05-A`'s clauses 2 and 3 must be amended against the wire's outcome set, and the remaining arms criterion 3 separates — reported failure, and spoke-and-stopped — have not been driven on a board over this channel. That is one small board-session piece, not a design question. **Not Verified.**
