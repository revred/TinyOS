...
Status: **Verified (functional) 2026-08-05 — every criterion Green 2026-08-03: the count was taken through the case seam the same evening it was written, and it read 3 — `pcie-link-down`, the data-link-active gate, is the Ethernet chain's first refused rung on this board. The same observation exposed the lamp's true polarity (`STORY-P1-07-08`, amended). Criterion 4 asks that the blink count name the rung and that the following story fix that rung by name, and both halves happened: the counted 3 selected `STORY-P1-09-08`, and the lamp went on to spell the deeper rungs the ladder reached (code 16 at `BOARD VERDICT 1`, then the plain unrefused pulse at `BOARD VERDICT 2`) — so the confession was read at three distinct depths and stopped when the chain cleared, which is the criterion's whole purpose rather than a single reading of it. Advanced under [`06A`](../../session/hand-2026-08-05/06A-nothing-is-verified-and-the-reason-is-not-velocity.md) §4.1: "Not Verified pending the assurance pass" was never a third option — the assurance pass is `specified` → `verified`, a different ladder from the functional `Status:` header, and holding the header for it conflated the two ceilings `06A` §2 separates. This Story's own header was also one of the seven that provoked `06A` §4.2's gate, now written. **Assurance state remains `specified` and this Story is NOT release-assured**: 0 qualified platforms.**
