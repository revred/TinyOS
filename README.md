...
**Which architecture carries which claim** ([ADR 0005](docs/adr/0005-arm64-real-time-tier-is-conditional-on-secure-world-qualification.md), accepted 2026-07-28, superseding [ADR 0004](docs/adr/0004-arm64-is-the-real-time-tier.md)): **worst-case latency bounds, WCET claims and jitter envelopes are quotable only from a *qualified* ARM64 platform** — one holding a dated secure-world qualification record naming its firmware version. x86_64 is disqualified outright, because System Management Interrupts are entered above the kernel and cannot be masked, observed or attributed by the OS, which makes any x86_64 worst-case bound a claim about the firmware rather than about TinyOS. ARM64 is not exempt by default: a GIC's secure interrupt groups routed to `EL3` preempt NS-EL1 irrespective of `PSTATE.I`, which is the same hole — but on ARM64 it is a *platform* property that a per-board campaign can bound, not an architecture property. **As of this writing no platform is qualified**, the Raspberry Pi 5 included, so an ARM64 run today is mechanism evidence, not a bound. **x86_64 remains a full first-class target** for throughput, rich-workload, host-bridge and developer-experience claims, and remains the Tier 0 CI gate. Nothing is de-scoped, and **no existing measurement is retracted or reinterpreted** by either ADR; what changes is which evidence a *bound* may be quoted from.
...
New here? If you're a coding agent, start with [`agent.md`](agent.md). If you're a human, start with [`SeedMVP.md`](SeedMVP.md) for the founding intent, then the latest dated handover under [`session/`](session/) — currently [`session/hand-2026-07-28/index.html`](session/hand-2026-07-28/index.html) — for a snapshot of what's decided, what's open, and what to work on next. For traceable, testable work items, see [`goals/`](goals/).
