...
- **G-PA-1: Deterministic actuation.** Every physical actuation command TinyOS issues runs against a **declared** decision-to-actuation budget that the scheduler **enforces**, and every violation of that budget is detected and reported — enforcement firing, not merely clean runs observed in testing. *Amended 2026-08-06 by [`ADR 0015`](docs/adr/0015-the-real-time-claim-is-enforcement-not-a-measured-worst-case.md). The original read "has a bounded, tested worst-case latency from decision to actuation, and that bound is enforced by the scheduler, not merely observed in testing" — and its second clause was always the provable half. The first claimed the system **has** a worst case, which no quantity of sampling establishes: an observed maximum over n=1000 is a different quantity from a bound. TinyOS declares and enforces a budget; it does not claim a bound. When a platform holds a secure-world qualification record (`ADR 0005`, zero hold one today), a sourced bound becomes an additional statement alongside this one rather than a replacement for it.*
