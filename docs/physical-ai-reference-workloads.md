...
- The Motion & Interpolation Service consumes position feedback through a `PositionFeedback` trait, never a concrete encoder driver type — Dependency Inversion, applied concretely. Today, in the absence of bolted-on physical encoders, Tier 0 testing (QEMU/Renode) supplies a **simulated feedback source** implementing the same trait, generating physically-plausible position data (including realistic error/lag) so the interpolation and closed-loop control logic is exercised exactly as it will be once real hardware arrives.
...
- The closed-loop control structure (position error computation, feedback consumption via the `PositionFeedback` trait) is real and exercised end-to-end against simulated feedback in Tier 0 testing — it is not a mock that gets thrown away when real encoders arrive.
...
- **Explicitly deferred, hardware-dependent**: physical positional/deposition/cure-quality accuracy validation for all three workloads, pending the real motion I/O, encoder, wire-feed, energy-source, and UV-array peripheral hardware referenced in [`TinyOS26thJulySeedMVP.md`](../TinyOS26thJulySeedMVP.md) as bolt-on additions to the MVP compute hardware.

## Status

This document accompanies Section 3.1 (Physical AI Goals) and Section 5 (MVP narrowing) of [`TinyOS26thJulySeedMVP.md`](../TinyOS26thJulySeedMVP.md), and is the concrete elaboration of the README's Physical AI deployment-mode references. It will be revised as the kinematics modules, process-synchronization functions, and interlock wiring for each workload move from specification to implementation.
