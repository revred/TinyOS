...
## Model storage & fast-load path (mmap/pointer-based, not read-and-copy)

Directed by the project (2026-07-26 session), citing `C:\Code\Sharc.Workspace\Sharc.Blue`'s `.rac` substrate as the prior-art pattern to study: the Ollama-like runtime's access to model weights and any other large on-SSD artifact should be **pointer-based against a memory-mapped file, not a read()-into-a-fresh-buffer copy** — the same shape as Sharc.Blue's `Blue.Sharc`/`Blue.Guard` `.rac` BlockStore, where `rac-ptr`-style lookups dereference directly into a warm-mmap'd region instead of parsing/copying on every access.

A few things worth getting precisely right before this is designed for real, so a future session doesn't overclaim what the pattern actually buys:

- **The "ns" numbers in Sharc.Blue's own docs describe warm/page-cache-resident access, not cold SSD reads.** `mmap` turns "give me bytes at this file offset" into a page-fault-then-pointer-dereference the first time a page is touched (still bound by real SSD/NVMe latency, microseconds-to-low-milliseconds, not ns), and a plain pointer dereference on every subsequent access to an already-faulted-in page (that's where the RAM-parity/sub-RAM-latency numbers come from — the OS page cache, not a physics-defying SSD). The right framing for TinyOS's inference model-load path is: **pay the SSD latency once per page, on first touch; every subsequent access is a bare pointer dereference with no syscall, no copy, no parse** — not "SSD access at ns speed" as a literal claim.
- **This needs a `no_std`/kernel-level mmap-equivalent**, not a `std::fs`/OS-hosted mmap — TinyOS's `inference` crate runs under TinyOS's own kernel (per the crate map), so the memory-mapping primitive is whatever the kernel's own virtual memory manager (`FEAT-P0-03` onward) exposes, plumbed through to the Unified Memory Manager above so a model-weight page can be demand-paged in from the storage device and handed to the GPU/CPU inference path without an intermediate copy.
- **Pointer-based access to untrusted model files is a security-boundary question, not just a performance one** — a model file is data crossing into a supervised operator's address space (Design Pillar 5); the mmap path must not become a way to defeat the ACI-mediated boundary the rest of this document establishes. Bounds/format validation happens before any pointer derived from the mapped region is trusted, exactly like Sharc.Blue's own codec validates `.rac` block headers before trusting pointer offsets into them.
- **Scope this to Phase 6** (this document's own Roadmap phase) once the Unified Memory Manager and kernel virtual-memory work it depends on (`FEAT-P0-03`) actually exist — this section records the *direction*, not a design ready to implement against Phase 0's walking skeleton.
