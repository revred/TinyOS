...
/os/                       The OS project — everything below this line compiles
  ...
  /targets/                Custom Rust target-spec JSON files for bare-metal x86_64/ARM64 builds (build data, not source)
  /src/                    ALL code — every crate, no exceptions
    /kernel/
    /hal/
    /hal-x86_64/
    /hal-arm64/
...
`os/targets/` is the one exception kept a sibling of `os/src/` rather than inside it: target-spec JSON files are build *data* consumed by `cargo build --target`, not Rust (or any) source code, so they fall outside "all code lives in `os/src`" the same way `Cargo.toml` itself does.

## Crate map

Every entry below is an `os/src/<name>/` directory with its own `Cargo.toml`. The **Phase** column ties each crate to the Roadmap in [`README.md`](../README.md#roadmap) and Section 10 of the seed specification, so this table is the concrete elaboration of that alignment, not a competing schedule.
...
| `motion` | Motion & Interpolation, Process-Synchronized Output, Position Feedback Abstraction, Safety Interlock — the shared RT primitives from `docs/physical-ai-reference-workloads.md` — plus the committed trunnion-table RTCP/TCPC kinematics module as a swappable submodule within it, per the Open/Closed pattern | Yes | Permitted | Phase 2–3, grown through the CNC milestone |
...
1. **Workspace bootstrap.** `Cargo.toml` workspace manifest, `rust-toolchain.toml`, `rustfmt.toml`, `deny.toml`, empty `kernel` and `hal`/`hal-x86_64` crates, `os/targets/x86_64-tinyos.json`, and `xtask` with a `build` and `qemu-x86_64` command. Success criterion: `cargo run -p xtask -- qemu-x86_64` boots to a halt with no panic, in CI.
