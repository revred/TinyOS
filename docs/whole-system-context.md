...
**Reviewed from source** in [`tauri-internals-review.md`](tauri-internals-review.md) (against `dev` at `872428f`), which maps every mechanism onto `PD-01`…`PD-14`. Four things that paragraph asserted are now evidenced, and two are sharper than it stated:

- **Caller identity is derived the right way.** Tauri resolves the calling window/webview from its own Rust-side objects, never from the message payload — `PD-02`'s shape, reached independently. The exception is the `__TAURI_INVOKE_KEY__` bearer secret that separates the app's frame from other frames; that one *is* caller-supplied and is not a boundary.
- **Origin really is dynamic.** Origin is computed from the webview's current URL at each call, so a navigation to a remote origin changes authority on the next call. The Charter's "a local webview that navigates to a remote origin loses local application IPC authority" is an implemented upstream mechanism, not a requirement TinyOS would have to add.
- **Capabilities are resolved at *build time*** — plugin permission manifests × app capability files, baked into the binary by `tauri-codegen`. This is a better fit for a signed manifest than a runtime permission model, and it is the strongest single point of alignment. The default posture still inverts against `PD-03`: `Capability.local` defaults to `true`, where TinyOS requires empty authority first — and a `Capability` may also bind commands to **remote** URL-pattern contexts, which the manifest intersection strips entirely: no `remote` grant survives, per the Charter's rule that remote content holds no application IPC authority.
- **The OS binding is already a trait seam.** `tauri-runtime` abstracts windowing and webview; `tauri-runtime-wry` is the sole implementation over `wry`/`tao`. A TinyOS port is `impl Runtime for TinyOsRuntime` rather than a fork — well-shaped, but not small: Tauri ships no renderer, so the port still requires the window/input service *and* a browser engine (`EPIC-H3`).

**The governing finding is the process model.** Tauri's core, `wry` and `tao` are one process, and every command handler runs there with the app's full OS authority. Tauri's ACL decides whether a string reaches a handler; it does not constrain what the handler then does. It is an in-process input filter, not an OS boundary — which is why `agent.md` rule 10 ("runtime permissions never replace the OS Protection Domain") is load-bearing here rather than boilerplate. The division of labour `APP-05` assumes is the correct one: **Tauri supplies interface shape — identity, typed commands, origin, enumerable build-time authority; TinyOS supplies isolation, accounting and time.**

**Modifying Tauri is in scope**, decided in [`ADR 0007`](adr/0007-modifying-tauri-is-in-scope-at-the-seams.md): fork at the seams under a patch-size discipline, baseline a release tag rather than `dev`, keep the fork outside the `os/` workspace, and carry a named advisory/rebase process. The review's §7 holds the analysis; the ADR holds the decision.
